...
两章同时规划且协同确实能节省工作时，可选用[问题模型章与求解方法章协同章节规划](problem-method-joint-writing.md)，但不要求另行交付接口台账。

若排程构造关系、完工时间递推、目标函数和可行性关系属于问题定义或数学模型，而非算法选择，应保留在本章。为后续复用的公式设置稳定标签，并记录求解方法所需的接口：直接编码的决策、解码得到的决策、可行性条件和评价输出。方法章应交叉引用这些定义，不应重复推导或重新编号。

算法专用的解码器、修复算子、时序子问题、增量评价器，或其他改变候选解构造/评价方式的过程，即使使用模型符号，也应在求解方法中说明。章节归属可能含混时，应明确写出这一区别。
