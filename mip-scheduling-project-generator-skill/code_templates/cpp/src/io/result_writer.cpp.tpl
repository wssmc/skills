...
void write_outputs(const SolveResult &solved, const std::filesystem::path &output_dir) {
    std::filesystem::create_directories(output_dir);
    {
        ...
        output << std::setprecision(17) << R"({"instance_id":")" << solved.result.instance_id
               << R"(","instance_seed":)" << solved.result.instance_seed << R"(,"algorithm":")"
               << solved.result.algorithm << R"(","round":)" << solved.result.round
               << R"(,"solve_seed":)" << solved.result.solve_seed << R"(,"objective":)";
        if (solved.result.feasible && std::isfinite(solved.result.objective)) {
            output << solved.result.objective;
        } else {
            output << "null";
        }
        output << R"(,"runtime_seconds":)" << solved.result.runtime_seconds << R"(,"feasible":)"
               << (solved.result.feasible ? "true" : "false") << R"(,"status":")"
               << solved.result.status << R"(","best_bound":)";
    ...
    }
    {
        ...
        for (std::size_t index = 0; index < solved.schedule.operations.size(); ++index) {
            ...
            output << R"({"job_id":)" << operation.job_id << R"(,"stage_id":)" << operation.stage_id
                   << R"(,"machine_id":)" << operation.machine_id << R"(,"start":)"
                   << operation.start << R"(,"end":)" << operation.end << '}';
        }
        output << "]}" << '\n';
    }
    {
